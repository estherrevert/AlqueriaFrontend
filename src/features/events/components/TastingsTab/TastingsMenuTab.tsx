...
export default function TastingsMenuTab({ eventId }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setError(e instanceof Error ? e.message : "Error cargando catas");
      } finally {
  ...
  if (error) return <div className="text-sm text-alert">{error}</div>;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-base font-bold text-text-main">Pruebas de menú</h3>
        <button
          type="button"
          className="rounded-xl bg-secondary px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-secondary-hover"
      ...
      <div className="bg-white border border-neutral-200 rounded-xl p-4 shadow-sm space-y-4">
        {items.length === 0 ? (
          <div className="text-sm text-muted">
            Aún no hay pruebas de menú asociadas al evento.
          </div>
        ) : (
